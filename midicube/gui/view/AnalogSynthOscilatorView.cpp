...
void property_mod_controls(std::vector<Control*>* controls, int x, int y, PropertyHolder* holder, size_t prop, std::string name, std::vector<Control*>* show_amount, std::vector<Control*>* show_source) {
	...
	//Mod Env
	{
		...
		DragBox<int>* source = new DragBox<int>(0, 0, ANALOG_SYNTH_PART_COUNT - 1, main_font, 16, x, y + 15, 80, 40);
	...
	//CC
	{
		...
		DragBox<int>* source = new DragBox<int>(0, 0, ANALOG_SYNTH_CONTROL_COUNT - 1, main_font, 16, x, y + 15, 80, 40);
...
Scene AnalogSynthOscilatorView::create(Frame &frame) {
	...
	//Filter
	{
		...
		active->bind(part, SynthPartProperty::pSynthOscFilter);
	...
	//Filter Type
	{
		...
		filter_type->bind(part, SynthPartProperty::pSynthOscFilterType);
	...
	//KB Track
	{
		...
		value->bind(part, SynthPartProperty::pSynthOscFilterKBTrack);
		controls.push_back(value);
	}
	tmp_x += 90;
	//KB Track Note
	{
		...
		value->bind(part, SynthPartProperty::pSynthOscFilterKBTrackNote);
		controls.push_back(value);
	}
	tmp_x += 90;
	tmp_y += 75;
	...
	tmp_x = 500;
	//Filter Cutoff
	property_mod_controls(&controls, tmp_x, tmp_y, part, SynthPartProperty::pSynthOscFilterCutoff, "Cutoff", &show_amount, &show_source);
	tmp_y += 75;
	//Filter Resonance
	property_mod_controls(&controls, tmp_x, tmp_y, part, SynthPartProperty::pSynthOscFilterResonance, "Resonance", &show_amount, &show_source);
	tmp_y += 75;
